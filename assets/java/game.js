...
$(document).ready(function () {
    ...
    for (i = 0; i < myobj; i++) {
        ...
        newcolumn.attr("attack", fighters.baseAttack[i]);
    ...
    $("body").on("click", ".selectable", function () {
        if (playerChosen === false) {
            ...
            attack = parseInt($(this).attr("attack"));
            newattack = attack;
    ...
    $("body").on("click", ".attackbtn", function () {
        ...
        if (player <= 0) { //losing condition
            $("#header").text("You have lost the game!");
            setTimeout(function () {
                location.reload();
            }, 3000);
        }
        else if (opponent <= 0 && defeated < 2) { //after you defeat an emeny
            defeated++;
        ...
        else if (opponent <= 0 && defeated >= 2) { //if youve already defeated both enemies + the third
        ...
        attack += newattack; //increment attack by its base
    });
